    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        lifecycleScope.launch {
            viewModel.filteredList.collect {
                Log.e( "onViewCreated2: ", it.size.toString())
                updateAdapter(it)
            }
        }
        ...
        lifecycleScope.launch {
            viewModel.stateOrder.collect {
                when {
                    ...
                    it.status == "success" -> {
                        loadDialogBar.hide()
                        val filterList = it.order?.allOrders?.filter { it.isCanceled == true && it.isDelivered == false }
                        if (filterList != null) {
            ...
            popupMenu.setOnMenuItemClickListener { item ->
                when (item.itemId) {
                    ...
                    R.id.action_sort_by_price_asc -> {
                        ...
                        Log.e( "addMenu2: ",sortedList.size.toString() )
                        viewModel.updateFilteredList(sortedList)
                        true
                    }
